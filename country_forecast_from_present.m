...
fprintf( '\nBasic Reproduction Number (BRN) is %f \n', BRN );
fprintf( 'Alpha is %f \n', param_fit.alpha );
fprintf( 'Beta is %f \n', param_fit.beta );
fprintf( 'incubation period is %f days \n', 1/param_fit.gamma);
fprintf( 'Infectious period  %f days \n', 1/param_fit.delta);
...
% param_fit.delta = 1 / 2.6; % Argentina 31/03/2020
% fprintf( 'Infectious period %f days \n', 1/param_fit.delta);
...
ll = legend([q1, r1, d1, qr, rr, dr], leg{:}, 'Location','NorthWest'); % 'Country','SouthWest'

title_srt = sprintf('%s %s, SEIR model is fitted with \n%d days, forecasted %d days from today', Country, City, DAYS, FORECAST);
tl =  title(title_srt);
...
fore_str = sprintf('and forecasts from %s to %s.', datestr( time(DAYS) + datenum(1) ), datestr( time_sim(end) ) );
infec_fore_srt = sprintf('It is predicted %d confirmed cases on %s.',  round( (Q1(end)) ), datestr( time_sim(end) ) );

text_box = sprintf('%s\n%s\n%s\n%s\n%s', days_str , fore_str, infec_fore_srt);

annotation('textbox', [0.14, 0.48, 0.1, 0.1], 'string', text_box, ...
    'LineStyle','none',...
    'FontSize', 20,...
    'FontName','Arial');
%     'FontWeight','bold',...
